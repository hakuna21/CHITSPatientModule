...
/**
 * Views an existing pelvic exam record of the given patient's current or previous family planning program.
 * 
 * @author Bren
 */
@Controller
@RequestMapping(value = "/module/chits/consults/viewFamilyPlanningPelvicExamination.form")
public class ViewPelvicExaminationController {
	...
	public FamilyPlanningConsultEntryForm formBackingObject(HttpServletRequest request, //
			ModelMap model, //
			@RequestParam(required = true, value = "pelvicExamObsId") Integer pelvicExamObsId, //
		...
		// load the pelvic exam record observation for display
		final Obs obs = obsService.getObs(pelvicExamObsId);

		// perform some sanity and security checks before allowing the user to view this observation
		if (patient != null && obs != null
				&& obs.getConcept().getConceptId().equals(FPPelvicExaminationConcepts.PELVIC_EXAMINATION.getConceptId()) //
			...
			// initialize the pelvic exam record to use for viewing
			form.setFpProgramObs(new FamilyPlanningProgramObs(obs.getObsGroup()));

			// allow user to view this observation
			form.getFpProgramObs().setPelvicExamination(new PelvicExamination(obs));
		} else {
	...
	public String showForm() {
		return "/module/chits/consults/familyplanning/chartfragments/pelvicExamination";
	}
