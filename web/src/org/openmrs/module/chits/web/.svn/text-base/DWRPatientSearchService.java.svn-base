...
/**
 * Specialized DWR service for searching patients.
 */
public class DWRPatientSearchService {
	/** Logger instance */
	...
	public Map<String, Object> findCountAndPatients(String searchValue, Integer start, Integer length, boolean getMatchCount) throws APIException {
		// Map to return
		...
		try {
			...
			if (getMatchCount) {
				patientCount += patientSearchSvc.getCountOfPatients(searchValue);
				...
				if ((length == null || length > 2) && patientCount < 3 && !searchValue.matches(".*\\d+.*")) {
					...
					if (!newSearch.equals(searchValue)) {
						...
						Collection<Patient> patients = patientSearchSvc.getPatients(searchValue);
						newSearch = newSearch.trim();
						Collection<Patient> newPatients = patientSearchSvc.getPatients(newSearch);
	 ...
	 */
	@SuppressWarnings("unchecked")
	private Collection<Object> findBatchOfPatients(String searchValue, boolean includeVoided, Integer start, Integer length) {
		if (maximumResults == null)
			maximumResults = Util.getMaximumSearchResults();
		if (length != null && length > maximumResults)
			length = maximumResults;
		...
		try {
			patients = patientSearchSvc.getPatients(searchValue, start, length);
		...
		if ((length == null || length > 2) && patients.size() < 3 && !searchValue.matches(".*\\d+.*")) {
			...
			if (!newSearch.equals(searchValue)) {
				Collection<Patient> newPatients = patientSearchSvc.getPatients(newSearch, start, length);
