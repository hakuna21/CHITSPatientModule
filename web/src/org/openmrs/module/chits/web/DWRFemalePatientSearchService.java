...
/**
 * Specialized DWR service for searching patients (e.g., searching only 'Female' patients).
 */
public class DWRFemalePatientSearchService {
	/** Logger instance */
	...
	public Map<String, Object> findCountAndFemalePatients(String searchValue, Integer start, Integer length, boolean getMatchCount) throws APIException {
		// Map to return
		...
		try {
			...
			if (getMatchCount) {
				patientCount += patientSearchSvc.getCountOfFemalePatients(searchValue);
				...
				if ((length == null || length > 2) && patientCount < 3 && !searchValue.matches(".*\\d+.*")) {
					...
					if (!newSearch.equals(searchValue)) {
						...
						Collection<Patient> patients = patientSearchSvc.getFemalePatients(searchValue);
						newSearch = newSearch.trim();
						Collection<Patient> newPatients = patientSearchSvc.getFemalePatients(newSearch);
			...
			if (patientCount > 0 || !getMatchCount)
				objectList = findBatchOfFemalePatients(searchValue, false, start, length);
	 ...
	 */
	@SuppressWarnings("unchecked")
	private Collection<Object> findBatchOfFemalePatients(String searchValue, boolean includeVoided, Integer start, Integer length) {
		if (maximumResults == null)
			maximumResults = Util.getMaximumSearchResults();
		if (length != null && length > maximumResults)
			length = maximumResults;
		...
		try {
			patients = patientSearchSvc.getFemalePatients(searchValue, start, length);
		...
		if ((length == null || length > 2) && patients.size() < 3 && !searchValue.matches(".*\\d+.*")) {
			...
			if (!newSearch.equals(searchValue)) {
				Collection<Patient> newPatients = patientSearchSvc.getFemalePatients(newSearch, start, length);
