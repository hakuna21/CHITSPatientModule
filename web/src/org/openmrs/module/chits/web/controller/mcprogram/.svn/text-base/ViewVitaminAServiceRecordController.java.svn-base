...
/**
 * Views an existing vitamin a service record of the given patient's current or previous maternal care program.
 */
@Controller
@RequestMapping(value = "/module/chits/consults/viewVitaminAServiceRecord.form")
public class ViewVitaminAServiceRecordController {
	...
	public MaternalCareConsultEntryForm formBackingObject(HttpServletRequest request, //
			ModelMap model, //
			@RequestParam(required = true, value = "vitaminAServiceRecordObsId") Integer vitaminAServiceRecordObsId, //
		...
		// load the vitamin a service record observation for display
		final Obs obs = obsService.getObs(vitaminAServiceRecordObsId);
		...
		if (patient != null && obs != null && //
				...
				&& obs.getValueCoded().getConceptId().equals(MCServiceTypes.VITAMIN_A_SUPPLEMENTATION.getConceptId()) //
				&& obs.getPerson().getPersonId().equals(form.getPatient().getPersonId())) {
			// initialize the vitamin a service record to use for viewing
			form.setVitaminAServiceRecord(new VitaminAServiceRecord(obs));
		} else {
	...
	public String showForm() {
		return "/module/chits/consults/maternalcare/ajaxViewVitaminAServiceRecordDetails";
	}
