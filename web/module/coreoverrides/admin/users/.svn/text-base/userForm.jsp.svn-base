...
<%-- BTZ: Change to absolute path since this is relative to the core overrides folder --%>
<%@ include file="/WEB-INF/view/admin/users/localHeader.jsp" %>
<%-- BTZ: End --%>
...
<form id="thisUserForm" method="post" action="user.form" autocomplete="off">
	...
	<%-- BTZ: Display current authorized barangays --%>
	<fieldset>
		<legend><spring:message code="chits.admin.authorized.barangays"/></legend>
		<table>
			<tr>
				<td>Authorized<br/>Barangays</td>
				<td valign="middle">
					<div id="authorized_barangays">
					<c:forEach var="userBarangay" items="${authorizedBarangays}">
						<div id="authbrgy_${userBarangay.barangayCode}">
							<label for="cb_${userBarangay.barangayCode}">
								${userBarangay.barangayCode}
								<c:if test="${not empty barangays[userBarangay.barangayCode]}">
								(${barangays[userBarangay.barangayCode].name}, ${barangays[userBarangay.barangayCode].municipality.name})
								</c:if>
							</label>
						</div>
					</c:forEach>
					<c:if test="${empty authorizedBarangays}">
						<spring:message code="chits.admin.no.authorized.barangays"/>
					</c:if>
					</div>
				</td>
			</tr>
		...
		<%-- BTZ: Add link to the 'Manage Authorized Barangays' page --%>
		<openmrs:hasPrivilege privilege="Edit Users">
		<c:if test="${not empty user.userId}">
			<input type="button" value="<spring:message code="chits.admin.manage.authorized.barangays" />" onClick="document.location.href='${pageContext.request.contextPath}/module/chits/admin/users/manageAuthorizedBarangays.form?user=${user.userId}'" />
		</c:if>
		</openmrs:hasPrivilege>
		<%-- BTZ: End --%>
	</fieldset>
	<%-- BTZ: End of authorized barangays --%>
