...
/**
 * Views a post partum internal examination record from a past or present patient maternal care program.
 * 
 * @author Bren
 */
@Controller
@RequestMapping(value = "/module/chits/consults/viewPostpartumInternalExaminationRecord.form")
public class ViewPostpartumInternalExaminationRecordController {
	...
	public String showForm(HttpServletRequest request, //
			...
			@RequestParam(required = true, value = "postpartumInternalExaminationRecordObsId") Integer postpartumInternalExaminationRecordObsId) {
		...
		// load the post partum IE record observation for display
		final Obs obs = obsService.getObs(postpartumInternalExaminationRecordObsId);

		// perform some sanity and security checks before allowing the user to view this observation
		if (obs != null && obs.getObsGroup() != null //
				&& obs.getObsGroup().getConcept().getConceptId().equals(ProgramConcepts.MATERNALCARE.getConceptId()) //
				&& obs.getConcept().getConceptId().equals(MCPostpartumIERecordConcepts.POSTDELIVERY_IE.getConceptId()) //
			...
			// initialize the postpartum IE record
			form.setPostpartumInternalExaminationRecord(new PostpartumInternalExaminationRecord(obs));
		} else {
