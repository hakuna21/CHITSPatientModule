function updateFPSection(patientId, onUpdate) {
	var handler
	$j.ajax({url: 'viewFamilyPlanningProgram.form?patientId=' + patientId + '&section=program-details', cache: false, success: handler = function (data) {
...
function submitAjaxForm(form, patientId, onUpdate, divId) {
	...
	$j.ajax({url: form.action, type: 'POST', data: postData, success: handler = function (data) {
		...
		} else if (div.find("div.openmrs_error").size() == 0) {
			onUpdate(patientId)
		}
	}, error: function(jqXHR) {handler(jqXHR.responseText)}})
}
